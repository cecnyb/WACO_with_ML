...
class ResNetBase(nn.Module):
    ...
    def embed_super_schedule(self, y) :
        ...
        y_clamped = y.clone()
        y_clamped[:, 45] = y_clamped[:, 45].clamp(0, 5)
        pchk = self.parchunk(y_clamped[:, 45].long())
...
class MetaTransferModel(nn.Module):
    def __init__(self, in_channels, D=3):
        ...
        self.shared_backbone = ResNetBase(in_channels, 128, D)
    ...
    def forward(self,  x, x2):
        shared_features = self.shared_backbone.embed_sparse_matrix(x, x2)
        return self.head(shared_features)

    def forward_after_query(self, x, y):
        return self.shared_backbone.forward_after_query(x, y)
